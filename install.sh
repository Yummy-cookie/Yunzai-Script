...
usepython=$(grep -oP 'usepython\s*:\s*\"\K[^"]+' /gancm/cfg.cfg)
...
installxpmbot(){
              ...
              if ! [ -x "$(command -v wget)" ];then
               ...
               echo -e "${YELLOW} 默认: $python ${RES} ${RED} \n 我们需要python大于3.8到3.10的版本 请仔细核对 五秒后开始继续安装${RES}"
                   while true; do
                   echo -e "${YELLOW}如果提示安装失败 请先去 其他设置 添加ppa源 增加Python ppa源${RES}"
                   echo -e "${GREEN}1. python3.8 （不建议）${RES}"
                   echo -e "${GREEN}2. python3.9 （备选）${RES}"
                   echo -e "${GREEN}3. python3.10 （建议）${RES}"
                   echo -e "${GREEN}4. 我已经安装了以上三个任意一个Python版本${RES}"              
                   read -p "安装python版本(0~4)" yes
                   case "$yes" in
                   1)
                        if ! [ -x "$(command -v python3.8)" ];then
                              installapt python3.8
                              sed -i "s/usepython:".*"/usepython:\"python3.8\"/" /gancm/cfg.cfg        
                              jzbl                                                    
                        fi
                        break
                        ;;
                   2)
                        if ! [ -x "$(command -v python3.9)" ];then
                              installapt python3.9
                              sed -i "s/usepython:".*"/usepython:\"python3.9\"/" /gancm/cfg.cfg    
                              jzbl                          
                        fi
                        break
                        ;;
                   3)
                        if ! [ -x "$(command -v python3.10)" ];then
                              ...
                              sed -i "s/usepython:".*"/usepython:\"python3.10\"/" /gancm/cfg.cfg    
                              jzbl                                                        
                        fi
                        break
                        ;;
              ...
              if [ ${usepython} = ${K} ]; then
                    echo -e "${GREEN}当前已安装${YELLOW}$(python3.8 --version 2>/dev/null ) $(python3.9 --version 2>/dev/null ) $(python3.10 --version 2>/dev/null )${RES}"
                    echo 请输入以下python版本
                    ...
                    echo -e "${YELLOW}输入Python版本 作为后续默认执行版本 例如输入 \"python3.10\" 回车 ${RES}"
                    read -r upython
                    sed -i "s/usepython:".*"/usepython:\"$upython\"/" /gancm/cfg.cfg
                    jzbl              
              fi
